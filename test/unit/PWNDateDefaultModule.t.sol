...
import {
    PWNDateDefaultModule,
    PWNHub,
    PWNHubTags,
    IPWNDefaultModule, DEFAULT_MODULE_INIT_HOOK_RETURN_VALUE
} from "pwn/periphery/loan/module/default/PWNDateDefaultModule.sol";

using MultiToken for address;

abstract contract PWNDateDefaultModuleTest is Test {

    PWNDateDefaultModule defaultModule;
    ...
    function setUp() public virtual {
        defaultModule = new PWNDateDefaultModule(PWNHub(hub));
...
contract PWNDateDefaultModule_OnLoanCreated_Test is PWNDateDefaultModuleTest {

    function test_shouldFail_whenCallerIsNotActiveLoan() external {
        _mockHubTag(loanContract, PWNHubTags.ACTIVE_LOAN, false);

        vm.expectRevert(PWNDateDefaultModule.CallerNotActiveLoan.selector);
    ...
    function test_shouldFail_whenProposerDataIsInvalid() external {
        vm.prank(loanContract);
        vm.expectRevert(PWNDateDefaultModule.InvalidProposerDataLength.selector);
    ...
    function test_shouldFail_whenLoanAlreadyInitialized() external {
        ...
        vm.expectRevert(PWNDateDefaultModule.LoanAlreadyInitialized.selector);
        vm.prank(loanContract);
        defaultModule.onLoanCreated(loanId, abi.encode(1 days));
    }

    function testFuzz_shouldFail_whenTimestampIsInThePast(uint256 timestamp) external {
        vm.warp(100 days);

        timestamp = bound(timestamp, 0, block.timestamp - 1);

        vm.prank(loanContract);
        vm.expectRevert(PWNDateDefaultModule.PastDefaultTimestamp.selector);
        defaultModule.onLoanCreated(loanId, abi.encode(timestamp));
    }

    function testFuzz_shouldFail_whenTimestampNotInFutureEnough(uint256 timestamp) external {
        vm.warp(100 days);

        timestamp = bound(timestamp, block.timestamp, block.timestamp + defaultModule.MIN_DURATION() - 1);

        vm.prank(loanContract);
        vm.expectRevert(PWNDateDefaultModule.DurationTooShort.selector);
        defaultModule.onLoanCreated(loanId, abi.encode(timestamp));
    }

    function testFuzz_shouldStoreData(uint256 timestamp) external {
        timestamp = bound(timestamp, block.timestamp + defaultModule.MIN_DURATION(), type(uint256).max);

        vm.prank(loanContract);
        defaultModule.onLoanCreated(loanId, abi.encode(timestamp));

        assertEq(defaultModule.defaultTimestamp(loanContract, loanId), timestamp);
    }
...
contract PWNDateDefaultModule_IsDefaulted_Test is PWNDateDefaultModuleTest {
