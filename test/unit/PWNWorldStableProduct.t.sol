...
import { PWNWorldStableProductHarness } from "test/harness/PWNWorldStableProductHarness.sol";

using MultiToken for address;

abstract contract PWNWorldStableProductTest is Test {
    ...
    IWorldID worldID = IWorldID(makeAddr("worldID"));
    string appId = "appId";
    ...
    PWNWorldStableProductHarness product;
    PWNWorldStableProduct.Proposal proposal;
    PWNWorldStableProduct.AcceptorValues acceptorValues;

    function setUp() public virtual {
        product = new PWNWorldStableProductHarness(hub, revokedNonce, utilizedCredit, feedRegistry, IChainlinkAggregatorLike(address(0)), weth, worldID, appId, actionId);

        proposal = PWNWorldStableProduct.Proposal({
        ...
        acceptorValues = PWNWorldStableProduct.AcceptorValues({
            creditAmount: 10 ether,
            loanToValue: 5000,
            root: 1,
            nullifierHash: 2,
            proof: [uint256(3), 4, 5, 6, 7, 8, 9, 10]
        });
        ...
        vm.mockCall(address(worldID), abi.encodeWithSelector(IWorldID.verifyProof.selector), abi.encode(""));
    ...
    function _hashProposalTypedData(PWNWorldStableProduct.Proposal memory _proposal) internal view returns (bytes32) {
...
contract PWNWorldStableProductTest_getCollateralAmount_Test is PWNWorldStableProductTest {
    ...
    function test_shouldFail_whenLTVIsZero() external {
        vm.expectRevert(abi.encodeWithSelector(PWNWorldStableProduct.LoanToValueZero.selector));
...
contract PWNWorldStableProductTest_ProposalModule_Test is PWNWorldStableProductTest {

    function test_shouldReturnNameAndVersion() external {
        (string memory name, string memory version) = product.nameAndVersion();
        assertEq(name, "PWN World Stable Product");
...
contract PWNWorldStableProductTest_acceptProposal_Test is PWNWorldStableProductTest {

    function testFuzz_shouldFail_whenCallerIsNotProposedLoanContract(address caller) external {
        vm.assume(caller != loanContract);

        vm.expectRevert(
            abi.encodeWithSelector(PWNWorldStableProduct.CallerNotLoanContract.selector, caller, loanContract)
        );
        vm.prank(caller);
    ...
    function test_shouldFail_whenCallerNotTagged_ACTIVE_LOAN() external {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(PWNWorldStableProduct.AddressMissingHubTag.selector, loanContract, PWNHubTags.ACTIVE_LOAN)
        );
    ...
    function testFuzz_shouldFail_whenProposalExpired(uint256 timestamp) external {
        ...
        vm.expectRevert(abi.encodeWithSelector(PWNWorldStableProduct.Expired.selector, timestamp, proposal.expiration));
        vm.prank(loanContract);
        product.acceptProposal(loanId, acceptor, proposer, _proposalData());
    }
    ...
    function testFuzz_shouldFail_whenDurationShorterThanMin(uint256 duration) external {
        ...
        vm.expectRevert(abi.encodeWithSelector(PWNWorldStableProduct.DurationTooShort.selector));
        vm.prank(loanContract);
        product.acceptProposal(loanId, acceptor, proposer, _proposalData());
    }
    ...
    function test_shouldFail_whenMinCreditAmountZero() external {
        ...
        vm.expectRevert(abi.encodeWithSelector(PWNWorldStableProduct.MinCreditAmountNotSet.selector));
        vm.prank(loanContract);
        product.acceptProposal(loanId, acceptor, proposer, _proposalData());
    }
    ...
    function testFuzz_shouldFail_whenCreditLessThanMinCreditAmount(uint256 creditAmount) external {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                PWNWorldStableProduct.InsufficientCreditAmount.selector,
    ...
    function testFuzz_shouldFail_whenAcceptableLTVZero() external {
        ...
        vm.expectRevert(PWNWorldStableProduct.InvalidAcceptableLoanToValue.selector);
        vm.prank(loanContract);
        product.acceptProposal(loanId, acceptor, proposer, _proposalData());
    }
    ...
    function testFuzz_shouldFail_whenInvalidLLTV(uint256 lltv) external {
        ...
        vm.expectRevert(PWNWorldStableProduct.InvalidLiquidationLoanToValue.selector);
        vm.prank(loanContract);
        product.acceptProposal(loanId, acceptor, proposer, _proposalData());
    ...
    function testFuzz_shouldFail_whenInvalidLTV_whenLenderProposer(uint256 ltv) external {
        ...
        vm.expectRevert(PWNWorldStableProduct.InvalidLoanToValue.selector);
        vm.prank(loanContract);
        product.acceptProposal(loanId, acceptor, proposer, _proposalData());
    }
    ...
    function test_shouldVerifyWorldIDProof() external {
        vm.expectCall(
            address(worldID),
            abi.encodeWithSelector(
                ...
                acceptorValues.nullifierHash,
                _hashToField(abi.encodePacked(_hashToField(abi.encodePacked(appId)), actionId)),
                acceptorValues.proof
            )
        );
    ...
    function test_shouldFail_whenInvalidWorldIDProof() external {
        vm.mockCallRevert(address(worldID), abi.encodeWithSelector(IWorldID.verifyProof.selector), abi.encode("revert data"));
    ...
    function testFuzz_shouldReturnLoanTerms(uint256 creditAmount, bool isProposerLender) external {
        ...
        assertEq(uint8(terms.collateral.category), uint8(Category.ERC20));
...
contract PWNWorldStableProductTest_interest_Test is PWNWorldStableProductTest {
...
contract PWNWorldStableProductTest_isDefaulted_Test is PWNWorldStableProductTest {
...
contract PWNWorldStableProductTest_liquidate_Test is PWNWorldStableProductTest {
    ...
    function test_shouldFail_whenDataIsNotEmpty() external {
        vm.expectRevert(PWNWorldStableProduct.LiquidationDataNotEmpty.selector);
        vm.prank(loanContract);
